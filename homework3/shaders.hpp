...
public:
    ...
    void UpdateBufferData(const std::vector<obj_data::vertex> & vertices) {
        glBindVertexArray(vao);
        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(obj_data::vertex), vertices.data(), GL_STATIC_DRAW);
    }
...
public:
    ...
    BunnyShader() {
        // Init program:
        GLuint source_vertex_shader = BunnyShader::create_shader(GL_VERTEX_SHADER, vertex_shader_source);
        GLuint source_fragment_shader = BunnyShader::create_shader(GL_FRAGMENT_SHADER, fragment_shader_source);
        program = create_program(source_vertex_shader, source_fragment_shader);
    ...
    void Draw(
        ...
        { // Draw full scene
            ...
            for (obj_data::face_data face : bunny.faces) {
                glActiveTexture(GL_TEXTURE0);
                glBindTexture(GL_TEXTURE_CUBE_MAP, bunny_reflection_texture);
                glUniform1i(reflection_texture_location, 0);
